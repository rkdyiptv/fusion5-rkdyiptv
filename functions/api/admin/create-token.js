...
export async function onRequest(context) {
  ...
  try {
    ...
    const validDeviceLimits = [1, 2, 3, 4, 5, 10, 15, 20, 30, 50, 100, 'unlimited'];
    let deviceLimit = body.deviceLimit;
    if (deviceLimit !== 'unlimited') deviceLimit = parseInt(deviceLimit);
    if (!validDeviceLimits.includes(deviceLimit)) {
      return new Response(JSON.stringify({ success: false, error: 'Invalid device limit' }), {
        status: 400, headers: commonHeaders,
      });
    }
